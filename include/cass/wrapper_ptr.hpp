...
template<typename T>
template<typename U>
wrapper_ptr<T>::wrapper_ptr(const wrapper_ptr<U> &other)
    : res(other.res), cnt(other.cnt)
{
    if (cnt)
        cnt->value += 2;
...
template<typename T>
template<typename U>
wrapper_ptr<T>::wrapper_ptr(wrapper_ptr<U> &&other)
    : res(other.res), cnt(other.cnt)
{
    other.cnt = nullptr;
}
...
template<typename T>
template<typename U>
wrapper_ptr<T> & wrapper_ptr<T>::operator = (const wrapper_ptr<U> &other)
{
    if (this != &other) {
        detach();
        res = other.res;
...
template<typename T>
template<typename U>
wrapper_ptr<T> & wrapper_ptr<T>::operator = (wrapper_ptr<U> &&other)
{
    if (this != &other) {
        detach();
        res = other.res;

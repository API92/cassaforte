...
{
    return ::cass_statement_set_paging_state(p, result->backend());
}

error statement::set_retry_policy(cass::retry_policy *retry_policy)
{
    return ::cass_statement_set_retry_policy(p, retry_policy->backend());
}

error statement::set_custom_payload(custom_payload const *payload)
{
    return ::cass_statement_set_custom_payload(
...
{
    return ::cass_statement_bind_collection(p, index, collection->backend());
}

error statement::bind_collection_by_name(char const *name,
        cass::collection const *collection)
{
    return ::cass_statement_bind_collection_by_name(p, name,
            collection->backend());
}
...
error statement::bind_user_type_by_name_n(char const *name, size_t name_length,
        cass::user_type const *user_type)
{
    return ::cass_statement_bind_user_type_by_name_n(p, name, name_length,
            user_type->backend());
}
